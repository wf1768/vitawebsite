...
<script>
        ...
        bootbox.confirm("确定要删除选择的照片墙图片吗？<br> <font color='red'>" +
            ...
            if(result){
                $.ajax({
                    type:"post",
                    data: "id=" + id,
                    url:"<?php echo site_url('a/marking/single_remove_image')?>",
        ...
        bootbox.confirm("确定要删除选择的照片墙图片吗？<br> <font color='red'>" +
            ...
            if(result){
                $.ajax({
                    type:"post",
                    data: "id=" + str,
                    url:"<?php echo site_url('a/marking/multi_remove_image')?>",
                    success: function(data){
                        if (data) {
                            window.location.reload();
                        }
    ...
    function save() {
        ...
        $.ajax({
            type:"post",
            data: formData,
            url:"<?php echo site_url('a/marking/edit_image')?>",
            success: function(data){
                if (data) {
                    window.location.reload();
                }
...
<form class="form-horizontal" id="edit-form"><!-- /control-group -->
      <div class="control-group">
	         ...
	         <label class="control-label" for="factory">图片说明</label>
             <div class="controls">
...
<div class="span12">
<h4>关于我们<small>照片墙</small></h4>
</div>
</div>
<div class="row-fluid filter-block">
<div class="pull-right"><a class="btn-flat new-product"
	onclick="upload_multi('<?php echo base_url() ?>','a/marking/upload_image'	)">+
添加图片</a> 
